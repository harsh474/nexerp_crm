...
def get_activities(name: str):
	...
	elif frappe.db.exists("RE Property Unit", name):
		return get_property_unit_activities(name)
	else:
...
def get_property_unit_activities(name: str):
	if not frappe.has_permission("RE Property Unit", "read", name):
		frappe.throw(_("Not permitted"), frappe.PermissionError)

	get_docinfo("", "RE Property Unit", name)
	docinfo = frappe.response["docinfo"]
	unit_meta = frappe.get_meta("RE Property Unit")
	unit_fields = {
		field.fieldname: {"label": field.label, "options": field.options} for field in unit_meta.fields
	}

	doc = frappe.db.get_values("RE Property Unit", name, ["creation", "owner"])[0]
	activities = [
		{
			"activity_type": "creation",
			"creation": doc[0],
			"owner": doc[1],
			"data": _("created this property unit"),
			"is_lead": False,
		}
	]
	...
	for version in docinfo.versions:
		...
		if change := data.get("changed")[0]:
			field = unit_fields.get(change[0], None)
	...
	# RE Property Unit has no call-log integration, so notes/tasks/attachments
	# are just a direct lookup against this record - no call-linking to merge in.
	notes = get_linked_notes(name)
	tasks = get_linked_tasks(name)
	attachments = get_attachments("RE Property Unit", name)
	calls = []
